...
import { createRadio } from './radio-host.js'
...
export function apply(ctx) {
  ...
  // ---- 网络电台（radio）----
  // 电台库独立持久化到 ~/.dsh/dsh-music-plus-radio.json；全部读写走
  // /dsh-music-plus/radio*，由 lib/radio-host.js 处理（内置 CC0 快照 + 外部电台源
  // + 手填电台 + 探活）。设置页是它唯一的界面，播放仍复用现有 audio 引擎。
  const radio = createRadio({
    stateDir: async () => {
      const h = await getHome()
      return (process.env.DSH_HOME) || (h === null ? null : h + '/.dsh')
    },
    writeJson,
    readBody,
  })
  const serve = async (req, res) => {
    try {
      ...
      // 网络电台：整段 /radio* 交给 radio 模块（设置页的电台库读写）。
      if (pathname.startsWith('/dsh-music-plus/radio') && await radio.handle(pathname, req, res, url)) return
      // JSON API routes
  ...
  const tool = {
    ...
    description: '控制 DSH 本地音乐库的播放。播放时可按歌曲名/歌手关键词搜索并播放（不传 query 则播放第一首音乐），或按歌单名播放自建歌单（playlist 参数）；也可用 radio/radioCategory 播放网络电台（按台名关键词或电台风格，如 爵士、电子、新闻）；还可用 action 执行暂停/继续/停止/下一首/上一首。',
    parameters: {
      type: 'object',
      ...
      properties: {
        ...
        radio: { type: 'string', description: '网络电台：电台名关键词（如「Jazz24」「NTS」）。仅当 action 为 play（默认）时使用，优先级最高，可留空' },
        radioCategory: { type: 'string', description: '网络电台风格/分类关键词（如 爵士、电子、氛围、新闻、游戏），从该风格里选一台播放。与 radio 可同时用，可留空' },
    ...
    async execute(args) {
      ...
      const radioQuery = args && typeof args.radio === 'string' ? args.radio.trim() : ''
      const radioCategory = args && typeof args.radioCategory === 'string' ? args.radioCategory.trim() : ''

      // 网络电台：不依赖本地曲库（电台库自带 400+ 台），只要指名了电台/风格就走这条路。
      // 命中后只投一个 intent，真正的解析与播放由浏览器半完成（与本地音乐同一通道）。
      if (action === 'play' && (radioQuery !== '' || radioCategory !== '')) {
        const found = await radio.search(radioQuery, radioCategory, 5)
        if (found.stations.length === 0) {
          const names = found.categories
            .map((c) => (c.nameZh ? c.nameZh : c.name))
            .filter((n, i, a) => a.indexOf(n) === i)
            .slice(0, 12)
            .join('、')
          return {
            action, played: false, kind: 'radio', track: '', matches: 0, count: 0,
            notice: '电台库里没有匹配「' + (radioQuery || radioCategory) + '」的电台。现有风格：'
              + (names || '（暂无，可打开「设置 → 网络电台」添加电台源）'),
          }
        }
        const pick = found.stations[0]
        pendingIntent = { action: 'play', radioId: pick.id, name: pick.name, radioCategory: pick.categoryId }
        return {
          action, played: true, kind: 'radio', track: pick.name, matches: found.total, count: found.total,
          notice: '已请求播放电台「' + pick.name + '」' + (pick.quality ? '（' + pick.quality + '）' : '')
            + '。播放是直播流，无法快进/回退；若被浏览器拦截，请在播放条上点一次 ▶ 解锁。',
        }
      }
  ...
  ctx.systemPrompt.section({
    ...
    text: '本机已挂载 DSH音乐播放器：可用 music_play_plus 工具按关键词播放 ~/Music（或设置的目录）里的音乐，或按歌单名播放自建歌单（playlist 参数），或按台名/风格播放网络电台（radio / radioCategory 参数，如「放个爵士电台」）；并支持 action 暂停/继续/停止/下一首/上一首。',
  })
