...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PREF_PODCAST_PLAYBACK = 'dsh-music-podcast-playback'; // 播客播放进度（独立键，避免被本地音乐覆盖）
    ...
    const PREF_KEYS = new Set([
      ...
      PREF_PLAYBACK, PREF_PODCAST_PLAYBACK,
    ...
    function savePlayback() {
      ...
      if (scope.kind === 'podcast') {
        ...
        // 独立键保存：避免与本地音乐共用 PREF_PLAYBACK 而互相覆盖。只存最小字段（标题+URL），
        // 确保不会因体积超出 Host prefs 上限被丢弃。
        savePref(PREF_PODCAST_PLAYBACK, JSON.stringify({
    ...
    function loadPodcastPlayback() {
      const raw = loadPref(PREF_PODCAST_PLAYBACK);
      if (raw === null) return null;
      try { const p = JSON.parse(raw); if (p && typeof p.podId === 'string') return p; } catch (e) {}
      return null;
    }
    ...
    function stop() {
      ...
      clearPref(PREF_PODCAST_PLAYBACK);
      releaseWakeLock();
    }
    ...
    // Local and podcast live in separate keys; restore whichever was played most
    // recently (by ts), so a podcast play is never clobbered by an older local value.
    function restoreLatest(list) {
      const loc = loadPlayback();
      const pod = loadPodcastPlayback();
      const locTs = (loc && Number.isFinite(loc.ts)) ? loc.ts : 0;
      const podTs = (pod && Number.isFinite(pod.ts)) ? pod.ts : 0;
      try { console.info('[dsh-music-plus] restoreLatest → loc?', !!loc, 'locTs', locTs, '| pod?', !!pod, 'podTs', podTs); } catch (e) {}
      if (pod && (podTs >= locTs || loc === null)) void restorePodcastPlayback(pod);
      else if (loc) restorePlayback(loc, list);
    }
