...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // 网络电台：DSH 设置页里独立的一页（设置 → 网络电台）。电台的全部界面都在这
      // 一页里——播放面板与播放条不新增任何东西；上面两个 slot 保持原样。
      ctx.effect(() => slots.inject('settings.section', () => slots.register(
        { name: 'settings.section', id: 'music-radio', order: 60, label: '网络电台' },
        () => React.createElement(RadioSettings),
      )), 'music-player-plus: radio settings section');
