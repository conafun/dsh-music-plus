...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resolvePlayable(id) {
      ...
      // 电台：'r:<stationId>'；url 可能已被宿主解析过（清单型电台），直接可用。
      if (String(id).startsWith('r:')) {
        const st = radioStationById(String(id).slice(2));
        if (st === null) return null;
        return { id, name: st.name, url: st.url, artists: [], quality: st.quality || '' };
      }
      return trackById(id);
    }
    // ---- 网络电台（radio）----
    // 电台库快照由宿主 /dsh-music-plus/radio 提供。设置页（设置 → 网络电台）是它
    // 唯一的界面；引擎侧只用它来解析 'r:<id>' 播放对象、给出 next/prev 的范围，
    // 以及给直播流做断流重连——播放条/播放面板一行都不改。
    let radioSnap = null;
    function setRadioSnap(snap) {
      if (snap && snap.ok === true) { radioSnap = snap; return true; }
      return false;
    }
    async function loadRadio() {
      try {
        const snap = await jsonGet('/dsh-music-plus/radio');
        if (setRadioSnap(snap)) return radioSnap;
      } catch (e) { /* 离线/宿主未就绪：沿用上一次快照 */ }
      return radioSnap;
    }
    const radioStations = () => (radioSnap && Array.isArray(radioSnap.stations)) ? radioSnap.stations : [];
    const radioStationById = (id) => radioStations().find((s) => s.id === id) || null;
    const isRadioScope = (s) => ((s || store.scope || {}).kind === 'radio');
    // 电台范围的可见台，排序与设置页列表**完全一致**（分类顺序 → 台名），
    // 否则 next/prev 的顺序会和用户看到的列表对不上。
    function radioScopeList(scopeId, query) {
      const cats = (radioSnap && radioSnap.categories) || [];
      const order = new Map(cats.map((c, i) => [c.id, Number.isFinite(c.order) ? c.order : i]));
      const q = String(query || '').toLowerCase();
      const list = radioStations().filter((s) => {
        if (s.hidden || s.hls) return false;
        if (scopeId === 'fav') { if (!s.fav) return false; }
        else if (scopeId !== undefined && scopeId !== 'all' && scopeId !== '' && s.categoryId !== scopeId) return false;
        if (q !== '' && (s.name + ' ' + (s.categoryName || '') + ' ' + (s.homepage || '')).toLowerCase().includes(q) === false) return false;
        return true;
      });
      list.sort((a, b) => {
        const oa = order.has(a.categoryId) ? order.get(a.categoryId) : 999;
        const ob = order.has(b.categoryId) ? order.get(b.categoryId) : 999;
        return (oa - ob) || String(a.name).localeCompare(String(b.name));
      });
      return list;
    }
    // 直播流会断：1/2/4/8/12 秒退避重连，5 次仍失败就自动切下一台（直播没有「下一集」，
    // 但用户按下 next 的语义是「换一个台」，所以走 step）。
    let radioRetry = 0;
    let radioRetryTimer = null;
    const RADIO_RETRY_DELAYS = [1000, 2000, 4000, 8000, 12000];
    function clearRadioRetry() {
      if (radioRetryTimer !== null) { clearTimeout(radioRetryTimer); radioRetryTimer = null; }
      radioRetry = 0;
    }
    function radioReconnect() {
      if (radioRetryTimer !== null) return;
      if (radioRetry >= RADIO_RETRY_DELAYS.length) { clearRadioRetry(); step(1); return; }
      const delay = RADIO_RETRY_DELAYS[radioRetry];
      radioRetry += 1;
      set({ playing: false, error: '电台断流，' + Math.round(delay / 1000) + ' 秒后重连（第 ' + radioRetry + ' 次）…' });
    ...
    // 清单型电台（.pls/.m3u）的真实流地址由宿主解析并按台缓存；这里只把解析结果
    // 填回快照再照常播放（第二次点同一台就直接命中宿主缓存）。
    let radioResolving = null;
    async function resolveRadioThenPlay(id) {
      if (radioResolving === id) return;
      radioResolving = id;
      try {
        const st = radioStationById(id);
        if (st === null) return;
        const r = await jsonGet('/dsh-music-plus/radio/resolve?url=' + encodeURIComponent(st.url)).catch(() => null);
        if (r && r.ok && r.url) { st.url = r.url; st.resolved = r.url; st.playlist = false; }
        else { set({ error: '电台地址解析失败：' + ((r && r.error) || '未知错误') }); return; }
      } finally { radioResolving = null; }
      startPlay('r:' + id);
    }
    // 播放某台。scopeId 决定 next/prev 的范围（'all' / 'fav' / 分类 id）。
    function playStation(stationId, scopeId) {
      const st = radioStationById(stationId);
      if (st === null) return;
      if (st.hls === true) {
        set({ error: '「' + st.name + '」是 HLS(.m3u8) 直播流，浏览器原生播放器不支持，已跳过' });
        return;
      }
      clearRadioRetry();
      const id = scopeId || (isRadioScope() ? store.scope.id : 'all');
      set({ scope: { kind: 'radio', id } });
      startPlay('r:' + stationId);
    }
    ...
    function activeIds() {
      ...
      if (s.kind === 'radio') {
        return radioScopeList(s.id).map((x) => 'r:' + x.id);
      }
      return (store.tracks || []).map((t) => t.id);
    }
    function scopeKey() {
      ...
      if (s.kind === 'radio') return 'rad:' + s.id;
      return 'lib';
    }
    ...
    function savePlayback() {
      ...
      // 电台是直播流：没有进度可续播，也绝不能覆盖本地音乐/播客的断点记录。
      if (isRadioScope()) return;
    ...
    function startPlay(id) {
      // 电台：清单型（.pls/.m3u）先请宿主解析出真实流地址再放；同时清掉上一台的
      // 重连计时器，避免换台后旧计时器把播放拽回去。
      if (String(id).startsWith('r:')) {
        const st = radioStationById(String(id).slice(2));
        if (st === null) return;
        if (st.playlist === true && !st.resolved) { void resolveRadioThenPlay(st.id); return; }
      }
      clearRadioRetry();
    ...
    function stop() {
      ...
      restoredMusicPos = null;
      // 电台：停播必须同时掐掉断流重连计时器，否则几秒后它会把声音又拉回来。
      clearRadioRetry();
      audio.pause();
    ...
    function bindAudio() {
      ...
      const onPlay = () => { set({ playing: true, error: null }); radioRetry = 0; acquireWakeLock(); setupLiveViz(); resumeVizCtx(); };
      const onPause = () => { set({ playing: false }); savePlayback(); releaseWakeLock(); };
      const onEnded = () => {
        // 直播间「结束」= 源站把连接断了（不是播完）。走退避重连，别当成下一首。
        if (isRadioScope()) { radioReconnect(); return; }
      ...
      const onError = () => {
        // 电台的 error 绝大多数是断流/源站 5xx，同样交给重连逻辑（它会耗尽后切下一台）。
        if (isRadioScope()) { radioReconnect(); return; }
    ...
    function PlayerPanel() {
      ...
      const listBody = React.createElement('div', { className: 'dsh-music-list-body' },
        ...
        // 网络电台：整页就挂在这个页签里（面板是 display:none 常驻渲染的，所以这里
        // 按「切到该页签才挂载」来避免白白渲染几百行台列表；电台库快照本身缓存在
        // radioSnap 里，来回切页签不会重新拉取，也不会闪空列表）。
        React.createElement('div', { style: paneStyle('radio'), className: 'dsh-music-radio-pane' },
          s.tab === 'radio' ? React.createElement(RadioSettings, { panelRef }) : null),
      ...
      return React.createElement('div', { className: 'dsh-music-panel', ref: panelRef, style: rootStyle },
        ...
        React.createElement('div', { className: 'dsh-music-panel-body' },
          ...
          React.createElement('div', { className: 'dsh-music-tabs' },
            tabBtn('music', '本地音乐'), tabBtn('podcast', '播客'), tabBtn('radio', '网络电台'), tabBtn('config', '系统配置')),
          React.createElement('div', { className: 'dsh-music-panel-content' },
            s.tab === 'music' ? React.createElement(DirectorySetting, { panelRef }) : null,
            s.tab === 'music' ? musicSubTabs : null,
            // 音乐错误/扫描提示统一在主列表区上方显示；系统配置页与网络电台页不显示
            // 曲库扫描相关的错误/加载提示（电台自己的报错在电台页里显示）。
            s.error && s.tab !== 'config' && s.tab !== 'radio' ? React.createElement('div', { className: 'dsh-music-error' }, s.error) : null,
            s.tab !== 'config' && s.tab !== 'radio' && s.loading ? React.createElement('div', { className: 'dsh-music-loading' }, '扫描中…') : null,
    ...
    // ---- 设置 → 网络电台 ----
    // 电台的**全部**界面都在这里（DSH 设置页的一个 section，id 'music-radio'）。
    // 播放面板与播放条一行都不新增：点某台的 ▶ 直接用现有 audio 引擎播放，播放条
    // 照旧显示台名；直播流的断流/重连也在引擎里处理（见 radioReconnect）。
    // 约定：所有写操作都回传整份快照，所以一次往返就能刷新整个设置页。
    const RADIO_PROBE_BATCH = 12;

    function radioTimeAgo(at) {
      if (!Number.isFinite(at) || at <= 0) return '未检测';
      const d = Date.now() - at;
    ...
    function RadioSettings() {
      const s = useStore();
      const [snap, setSnap] = useState(radioSnap);
      const [q, setQ] = useState('');
      const [cat, setCat] = useState('all');
      const [busy, setBusy] = useState('');
      const [msg, setMsg] = useState(null);
      const [probe, setProbe] = useState(null);      // { done, total, alive }
      const [panel, setPanel] = useState('');        // '' | 'station' | 'source' | 'style'
      const [form, setForm] = useState({ id: '', name: '', url: '', homepage: '', categoryId: '', tags: '' });
      const [srcUrl, setSrcUrl] = useState('');
      const [newCat, setNewCat] = useState('');
      const [tab, setTab] = useState('all');         // 列表范围：all | fav | <categoryId> | hidden
      const stopRef = useRef(false);

      useEffect(() => {
        let alive = true;
        loadRadio().then((x) => { if (alive && x) setSnap(x); });
        return () => { alive = false; };
      }, []);

      const apply = (r) => { if (r && setRadioSnap(r)) setSnap(r); };
      const post = async (path, body) => {
        setBusy(path);
        try {
          const r = await jsonPost('/dsh-music-plus/radio' + path, body).catch(() => null);
          if (r === null) { setMsg({ ok: false, text: '宿主没有响应，请刷新页面重试' }); return null; }
          if (r.ok === false) {
            setMsg({ ok: false, text: r.error || '操作失败' });
            if (Array.isArray(r.stations)) apply(r);
            return null;
          }
          apply(r);
          return r;
      ...
      const categories = (snap && snap.categories) || [];
      const stats = (snap && snap.stats) || { stations: 0, categories: 0, probe: { ok: 0, dead: 0, unknown: 0 } };
      const catById = new Map(categories.map((c) => [c.id, c]));
      const orderOf = (id) => { const c = catById.get(id); return (c && Number.isFinite(c.order)) ? c.order : 999; };
      const catLabel = (id) => {
        const c = catById.get(id);
        if (!c) return '未分类';
        return c.nameZh ? c.nameZh : c.name;
      };

      // 当前列表（与引擎的 radioScopeList 同序：分类顺序 → 台名）
      const ql = q.trim().toLowerCase();
      let list = stations.filter((x) => {
        if (tab === 'hidden') return x.hidden === true;
        if (x.hidden === true) return false;
        if (tab === 'fav') return x.fav === true;
        if (tab !== 'all' && x.categoryId !== tab) return false;
        return true;
      });
      if (ql !== '') {
        list = list.filter((x) => (x.name + ' ' + (x.categoryName || '') + ' ' + (x.homepage || '') + ' ' + x.tags.join(' ')).toLowerCase().includes(ql));
      }
      list = list.slice().sort((a, b) => (orderOf(a.categoryId) - orderOf(b.categoryId)) || String(a.name).localeCompare(String(b.name)));
      const MAX_ROWS = 300;
      const shown = list.slice(0, MAX_ROWS);

      const currentRadioId = String(s.currentId || '').startsWith('r:') ? String(s.currentId).slice(2) : null;

      const playToggle = (x) => {
        if (currentRadioId === x.id) {
          if (s.playing) { audio.pause(); set({ playing: false }); }
          else { const p = audio.play(); if (p && p.catch) p.catch(() => {}); }
          return;
        }
        playStation(x.id, tab === 'all' || tab === 'hidden' ? x.categoryId : tab);
      };

      const probeList = async (items) => {
        if (items.length === 0 || probe !== null) return;
        stopRef.current = false;
        let done = 0, alive = 0;
        setProbe({ done: 0, total: items.length, alive: 0 });
        for (let i = 0; i < items.length; i += RADIO_PROBE_BATCH) {
          if (stopRef.current) break;
          const chunk = items.slice(i, i + RADIO_PROBE_BATCH).map((x) => x.id);
          const r = await jsonPost('/dsh-music-plus/radio/probe', { ids: chunk }).catch(() => null);
          if (r === null) { setMsg({ ok: false, text: '检测中断（宿主没有响应）' }); break; }
          done += chunk.length;
          alive += Number(r.alive) || 0;
          setProbe({ done, total: items.length, alive });
        }
        setProbe(null);
        const fresh = await loadRadio();
        if (fresh) setSnap(fresh);
        setMsg({ ok: true, text: '检测完成：' + done + ' 台，其中 ' + alive + ' 台在线' });
      };

      const submitStation = async () => {
        const name = form.name.trim(), url = form.url.trim();
        if (url === '') { setMsg({ ok: false, text: '请填写流地址' }); return; }
        const tags = form.tags.split(/[,，\s]+/).map((t) => t.trim()).filter((t) => t !== '');
        const body = { name, url, homepage: form.homepage.trim(), categoryId: form.categoryId, tags };
        const r = form.id !== '' ? await post('/station/update', { id: form.id, ...body }) : await post('/station/add', body);
        if (r !== null) {
          setMsg({ ok: true, text: form.id !== '' ? '已保存「' + (name || url) + '」' : '已添加「' + (name || url) + '」，可在列表里点 ▶ 播放' });
          setForm({ id: '', name: '', url: '', homepage: '', categoryId: '', tags: '' });
          setPanel('');
        }
      };
      ...
      const editStation = (x) => {
        setForm({
          id: x.custom ? x.id : x.id,
          name: x.name, url: x.url, homepage: x.homepage || '',
          categoryId: x.categoryId, tags: (x.tags || []).join(' '),
        });
        setPanel('station');
      };

      const deleteStation = (x) => {
        openConfirm(
          x.custom ? '删除电台' : '从列表移除',
          x.custom
            ? '确定删除「' + x.name + '」？这台是你自己添加的，删除后需要重新添加。'
            : '「' + x.name + '」来自内置库或外部源，无法真正删除，将改为隐藏（可在「已隐藏」里恢复）。',
          async () => {
            const r = await post('/station/remove', { id: x.id });
            if (r !== null) setMsg({ ok: true, text: x.custom ? '已删除' : '已隐藏「' + x.name + '」' });
          },
          x.custom ? '删除' : '隐藏', true,
        );
      };

      const styleRow = (c) => React.createElement('div', { className: 'dsh-music-radio-stylerow', key: c.id },
        React.createElement('span', { className: 'dsh-music-radio-stylename', title: c.name },
          (c.nameZh ? c.nameZh : c.name),
          React.createElement('em', { className: 'dsh-music-radio-dim' }, ' · ' + c.count + ' 台' + (c.custom ? ' · 自定义' : ''))),
        React.createElement('button', {
          className: 'dsh-music-settings-btn ghost small',
          onClick: () => post('/category/update', { id: c.id, hidden: !c.hidden }),
        }, c.hidden ? '显示' : '隐藏'),
        React.createElement('button', {
          className: 'dsh-music-settings-btn ghost small',
          onClick: () => { openPrompt('重命名风格', c.name, (v) => { void post('/category/update', { id: c.id, name: v }); }); },
        }, '改名'),
        React.createElement('button', {
          className: 'dsh-music-settings-btn ghost small',
          onClick: () => post('/category/update', { id: c.id, order: (c.order || 0) - 1.5 }),
        }, '↑'),
        React.createElement('button', {
          className: 'dsh-music-settings-btn ghost small',
          onClick: () => post('/category/update', { id: c.id, order: (c.order || 0) + 1.5 }),
        }, '↓'),
        c.custom ? React.createElement('button', {
          className: 'dsh-music-settings-btn ghost small danger',
          onClick: () => openConfirm('删除风格', '删除自定义风格「' + c.name + '」？该风格下的电台会回到「未分类」，电台本身不删。',
            () => { void post('/category/remove', { id: c.id }); }, '删除', true),
        }, '删除') : null,
      );

      const row = (x) => {
        const isCur = currentRadioId === x.id;
        const p = x.probe;
        const probeCls = p === null || p === undefined ? 'unknown' : (p.status === 'ok' ? 'ok' : (p.status === 'dead' ? 'dead' : 'unknown'));
        const probeTitle = p === null || p === undefined
          ? '尚未检测'
          : (p.status === 'ok' ? '在线 · ' + p.ms + 'ms · ' + radioTimeAgo(p.at) : '离线 · ' + (p.error || '') + ' · ' + radioTimeAgo(p.at));
        return React.createElement('div', { className: 'dsh-music-radio-row' + (isCur ? ' current' : '') + (x.hls ? ' hls' : ''), key: x.id },
          React.createElement('span', { className: 'dsh-music-radio-dot ' + probeCls, title: probeTitle }),
          React.createElement('button', {
            className: 'dsh-music-radio-play',
            title: x.hls ? 'HLS 直播流，浏览器原生播放器不支持' : (isCur && s.playing ? '暂停' : '播放'),
            disabled: x.hls === true,
            onClick: () => playToggle(x),
          }, isCur && s.playing ? '⏸' : '▶'),
          React.createElement('span', { className: 'dsh-music-radio-name', title: x.url },
            x.name,
            x.country ? React.createElement('em', { className: 'dsh-music-radio-dim' }, ' · ' + x.country) : null),
          tab === 'all' || tab === 'hidden' || ql !== ''
            ? React.createElement('span', { className: 'dsh-music-radio-badge' }, catLabel(x.categoryId))
            : null,
          x.hls ? React.createElement('span', { className: 'dsh-music-radio-badge warn' }, '需 HLS') : null,
          x.playlist ? React.createElement('span', { className: 'dsh-music-radio-badge' }, '清单') : null,
          x.quality ? React.createElement('span', { className: 'dsh-music-radio-badge' }, x.quality) : null,
          x.custom ? React.createElement('span', { className: 'dsh-music-radio-badge accent' }, '我的') : null,
          x.down === true ? React.createElement('span', { className: 'dsh-music-radio-badge danger' }, '源标记失效') : null,
          React.createElement('span', { className: 'dsh-music-radio-spacer' }),
          x.homepage ? React.createElement('a', {
            className: 'dsh-music-radio-link', href: x.homepage, target: '_blank', rel: 'noreferrer', title: '打开电台主页',
          }, '主页') : null,
          React.createElement('button', {
            className: 'dsh-music-radio-star' + (x.fav ? ' on' : ''),
            title: x.fav ? '取消收藏' : '收藏',
            onClick: () => post('/favorite', { id: x.id, fav: !x.fav }),
          }, x.fav ? '★' : '☆'),
          React.createElement('button', {
            className: 'dsh-music-radio-mini', title: '编辑',
            onClick: () => editStation(x),
          }, '✎'),
          React.createElement('button', {
            className: 'dsh-music-radio-mini', title: x.hidden ? '恢复显示' : (x.custom ? '删除' : '隐藏'),
            onClick: () => { if (x.hidden) void post('/station/update', { id: x.id, hidden: false }); else deleteStation(x); },
          }, x.hidden ? '恢复' : '✕'),
        );
      };

      // 列表按「风格」分组渲染（同序列表 → 连续同分类自然成组）
      const rows = [];
      let lastCat = null;
      for (const x of shown) {
        if (x.categoryId !== lastCat) {
          lastCat = x.categoryId;
          rows.push(React.createElement('div', { className: 'dsh-music-radio-group', key: 'g-' + x.categoryId },
            catLabel(x.categoryId),
            React.createElement('em', { className: 'dsh-music-radio-dim' }, ' · ' + list.filter((y) => y.categoryId === x.categoryId).length + ' 台')));
        }
        rows.push(row(x));
      }
      ...
      const chip = (key, label, count) => React.createElement('button', {
        key: key,
        className: 'dsh-music-radio-chip' + (tab === key ? ' active' : ''),
        onClick: () => setTab(key),
      }, label + (count === undefined ? '' : ' ' + count));

      const hiddenCount = stations.filter((x) => x.hidden === true).length;
      const favCount = stations.filter((x) => x.fav === true).length;
      const probeSummary = '🟢 ' + stats.probe.ok + ' · 🔴 ' + stats.probe.dead + ' · ⚪ ' + stats.probe.unknown;

      return React.createElement('div', { className: 'dsh-music-settings dsh-music-radio' },
        // 概览
        React.createElement('div', { className: 'dsh-music-radio-stats' },
          React.createElement('span', null, '共 ' + stats.stations + ' 台 / ' + stats.categories + ' 个风格（显示 ' + stats.visible + ' 台）'),
          React.createElement('span', { className: 'dsh-music-radio-dim' }, probeSummary),
          React.createElement('span', { className: 'dsh-music-radio-dim', title: snap && snap.builtin ? snap.builtin.url : '' },
            '内置库 ' + (snap && snap.builtin && snap.builtin.revision ? snap.builtin.revision.slice(0, 7) : '—')
            + (snap && snap.builtin && snap.builtin.updated ? '（已更新）' : '')),
          React.createElement('span', { className: 'dsh-music-radio-spacer' }),
          React.createElement('button', {
            className: 'dsh-music-settings-btn ghost small', disabled: busy !== '' || probe !== null,
            onClick: async () => { const x = await loadRadio(); if (x) { setSnap(x); setMsg({ ok: true, text: '已刷新' }); } },
          }, '刷新')),
        // 工具行
        React.createElement('div', { className: 'dsh-music-radio-tools' },
          React.createElement('input', {
            className: 'dsh-music-radio-search', value: q, placeholder: '搜索电台名 / 风格 / 主页',
            onChange: (e) => setQ(e.target.value), 'aria-label': '搜索电台',
          }),
          React.createElement('button', {
            className: 'dsh-music-settings-btn ghost small', disabled: probe !== null || list.length === 0,
            title: '检测当前列表里的电台是否还能连上（每批 ' + RADIO_PROBE_BATCH + ' 个并发）',
            onClick: () => { void probeList(list); },
          }, probe !== null ? '检测中 ' + probe.done + '/' + probe.total : '检测当前列表'),
          React.createElement('button', {
            className: 'dsh-music-settings-btn ghost small', disabled: probe !== null || stats.stations === 0,
            title: '检测全部电台（较慢，可随时停止）',
            onClick: () => { void probeList(stations.filter((x) => x.hidden !== true)); },
          }, '检测全部'),
          probe !== null ? React.createElement('button', {
            className: 'dsh-music-settings-btn ghost small danger',
            onClick: () => { stopRef.current = true; },
          }, '停止') : null),

        // 风格 chips
        React.createElement('div', { className: 'dsh-music-radio-chips' },
          chip('all', '全部', stats.stations),
          chip('fav', '★ 收藏', favCount),
          categories.filter((c) => !c.hidden || c.count > 0).map((c) => chip(c.id, c.nameZh ? c.nameZh : c.name, c.count)),
          hiddenCount > 0 ? chip('hidden', '已隐藏', hiddenCount) : null),

        // 提示 / 错误
        msg !== null ? React.createElement('div', { className: 'dsh-music-radio-msg' + (msg.ok ? ' ok' : ' err') }, msg.text) : null,
        s.error !== null && isRadioScope() ? React.createElement('div', { className: 'dsh-music-radio-msg err' }, s.error) : null,

        // 电台列表
        React.createElement('div', { className: 'dsh-music-radio-list' },
          shown.length === 0
            ? React.createElement('div', { className: 'dsh-music-empty' }, ql !== '' ? '没有匹配「' + q + '」的电台' : (tab === 'hidden' ? '没有已隐藏的电台' : (tab === 'fav' ? '还没有收藏电台，点列表行尾的 ☆ 收藏' : '这个风格下暂时没有电台')))
            : rows,
          list.length > shown.length
            ? React.createElement('div', { className: 'dsh-music-hint' }, '还有 ' + (list.length - shown.length) + ' 台未显示，请用搜索或选择其他风格缩小范围。')
            : null),

        // 折叠区：添加电台 / 电台源 / 风格管理
        React.createElement('div', { className: 'dsh-music-radio-sections' },
          React.createElement('button', {
            className: 'dsh-music-radio-sechead' + (panel === 'station' ? ' open' : ''),
            onClick: () => { setPanel(panel === 'station' ? '' : 'station'); if (panel !== 'station') setForm({ id: '', name: '', url: '', homepage: '', categoryId: '', tags: '' }); },
          }, '＋ 添加 / 编辑电台'),
          panel === 'station' ? React.createElement('div', { className: 'dsh-music-radio-secbody' },
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('label', null, '流地址（必填）'),
              React.createElement('input', {
                value: form.url, placeholder: 'http:// 或 https:// 的电台流地址',
                onChange: (e) => setForm({ ...form, url: e.target.value }),
              })),
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('label', null, '名称'),
              React.createElement('input', {
                value: form.name, placeholder: '留空则用域名',
                onChange: (e) => setForm({ ...form, name: e.target.value }),
              })),
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('label', null, '主页（可选）'),
              React.createElement('input', {
                value: form.homepage, placeholder: 'https://…',
                onChange: (e) => setForm({ ...form, homepage: e.target.value }),
              })),
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('label', null, '风格'),
              React.createElement('select', {
                value: form.categoryId, onChange: (e) => setForm({ ...form, categoryId: e.target.value }),
              },
                React.createElement('option', { value: '' }, '未分类'),
                categories.map((c) => React.createElement('option', { key: c.id, value: c.id }, c.nameZh ? c.nameZh : c.name)))),
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('label', null, '标签（可选，空格分隔）'),
              React.createElement('input', {
                value: form.tags, placeholder: '深夜 工作 本地',
                onChange: (e) => setForm({ ...form, tags: e.target.value }),
              })),
            React.createElement('p', { className: 'dsh-music-hint' }, '支持直接填 .pls / .m3u 清单地址（播放前由宿主解析出真实流地址）；.m3u8(HLS) 浏览器原生播不了，添加后会标为「需 HLS」。'),
            React.createElement('div', { className: 'dsh-music-settings-row' },
              React.createElement('button', { className: 'dsh-music-settings-btn', disabled: busy !== '', onClick: () => { void submitStation(); } },
                busy !== '' ? '保存中…' : (form.id !== '' ? '保存修改' : '添加电台')),
              React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: () => { setForm({ id: '', name: '', url: '', homepage: '', categoryId: '', tags: '' }); setPanel(''); } }, '取消'))) : null,

          React.createElement('button', {
            className: 'dsh-music-radio-sechead' + (panel === 'source' ? ' open' : ''),
            onClick: () => setPanel(panel === 'source' ? '' : 'source'),
          }, '电台源（内置库 / 外部源）'),
          panel === 'source' ? React.createElement('div', { className: 'dsh-music-radio-secbody' },
            React.createElement('div', { className: 'dsh-music-radio-srccard' },
              React.createElement('div', null,
                React.createElement('strong', null, snap && snap.builtin ? snap.builtin.name : '推荐网络电台'),
                React.createElement('em', { className: 'dsh-music-radio-dim' }, ' · 内置快照 · CC0 · ' + stats.stations + ' 台')),
              React.createElement('div', { className: 'dsh-music-radio-dim small' },
                '来源：' + (snap && snap.builtin ? snap.builtin.url : '') + (snap && snap.builtin && snap.builtin.updated ? '（已从 GitHub 更新过一次）' : '')),
              React.createElement('div', { className: 'dsh-music-settings-row' },
                React.createElement('button', {
                  className: 'dsh-music-settings-btn ghost small', disabled: busy !== '',
                  onClick: async () => {
                    const r = await post('/source/refresh', { id: 'builtin-recommended' });
                    if (r !== null) setMsg({ ok: true, text: '内置库已更新：' + r.categories + ' 个风格 / ' + r.stations + ' 台（你的收藏与隐藏都已保留）' });
                  },
                }, '从 GitHub 更新'),
                snap && snap.builtin && snap.builtin.updated ? React.createElement('button', {
                  className: 'dsh-music-settings-btn ghost small',
                  onClick: () => openConfirm('恢复内置库', '丢弃「从 GitHub 更新」的内容，恢复成插件自带的快照？你的收藏、隐藏与自定义电台不受影响。',
                    async () => { const r = await post('/source/reset', {}); if (r !== null) setMsg({ ok: true, text: '已恢复插件内置库' }); }, '恢复', false),
                }, '恢复内置') : null)),
            React.createElement('p', { className: 'dsh-music-hint' }, '外部源：填一个电台清单链接即可整批导入。支持 M3U/M3U8、PLS、XSPF、ASX，以及 radio-browser.info 的 JSON（如 https://de1.api.radio-browser.info/json/stations/bytag/jazz?limit=100）。'),
            React.createElement('div', { className: 'dsh-music-radio-field' },
              React.createElement('input', {
                value: srcUrl, placeholder: '电台清单 / JSON 链接',
                onChange: (e) => setSrcUrl(e.target.value),
              })),
            React.createElement('div', { className: 'dsh-music-settings-row' },
              React.createElement('button', {
                className: 'dsh-music-settings-btn', disabled: busy !== '' || srcUrl.trim() === '',
                onClick: async () => {
                  const r = await post('/source/add', { url: srcUrl.trim() });
                  if (r !== null) { setSrcUrl(''); setMsg({ ok: true, text: '已添加源「' + r.source.name + '」：' + r.source.count + ' 台（格式 ' + r.source.format + '）' }); }
                },
              }, busy !== '' ? '拉取中…' : '添加源'),
              React.createElement('span', { className: 'dsh-music-radio-dim small' }, '最多 20 个外部源')),
            (snap && snap.sources ? snap.sources : []).map((src) => React.createElement('div', { className: 'dsh-music-radio-srccard', key: src.id },
              React.createElement('div', null,
                React.createElement('strong', null, src.name),
                React.createElement('em', { className: 'dsh-music-radio-dim' }, ' · ' + src.count + ' 台 · ' + (src.format || '') + ' · ' + radioTimeAgo(src.refreshedAt))),
              React.createElement('div', { className: 'dsh-music-radio-dim small', title: src.url }, src.url),
              src.err !== '' ? React.createElement('div', { className: 'dsh-music-radio-msg err' }, src.err) : null,
              React.createElement('div', { className: 'dsh-music-settings-row' },
                React.createElement('button', {
                  className: 'dsh-music-settings-btn ghost small', disabled: busy !== '',
                  onClick: async () => { const r = await post('/source/refresh', { id: src.id }); if (r !== null) setMsg({ ok: true, text: '已刷新：' + r.count + ' 台' }); },
                }, '刷新'),
                React.createElement('button', {
                  className: 'dsh-music-settings-btn ghost small danger',
                  onClick: () => openConfirm('删除电台源', '删除「' + src.name + '」及它导入的 ' + src.count + ' 台电台？你的收藏与自定义电台不受影响。',
                    async () => { const r = await post('/source/remove', { id: src.id }); if (r !== null) setMsg({ ok: true, text: '已删除电台源' }); }, '删除', true),
                }, '删除'))))) : null,

          React.createElement('button', {
            className: 'dsh-music-radio-sechead' + (panel === 'style' ? ' open' : ''),
            onClick: () => setPanel(panel === 'style' ? '' : 'style'),
          }, '风格管理（' + categories.length + '）'),
          panel === 'style' ? React.createElement('div', { className: 'dsh-music-radio-secbody' },
            React.createElement('div', { className: 'dsh-music-settings-row' },
              React.createElement('input', {
                className: 'dsh-music-radio-search', value: newCat, placeholder: '新建风格，如「深夜」「通勤」',
                onChange: (e) => setNewCat(e.target.value),
                onKeyDown: (e) => { if (e.key === 'Enter' && newCat.trim() !== '') { void post('/category/add', { name: newCat.trim() }).then((r) => { if (r !== null) { setNewCat(''); setMsg({ ok: true, text: '已新建风格' }); } }); } },
              }),
              React.createElement('button', {
                className: 'dsh-music-settings-btn', disabled: newCat.trim() === '',
                onClick: async () => { const r = await post('/category/add', { name: newCat.trim() }); if (r !== null) { setNewCat(''); setMsg({ ok: true, text: '已新建风格' }); } },
              }, '新建')),
            React.createElement('p', { className: 'dsh-music-hint' }, '隐藏的风格不再出现在上面的选择条里（已隐藏的电台可以在「已隐藏」里恢复）。↑↓ 调整风格顺序，即电台列表的分组顺序。'),
            categories.map(styleRow)) : null),

        // 底部说明
        React.createElement('p', { className: 'dsh-music-hint' },
          '播放：点每行 ▶ 即可用播放条播放（直播流无法快进/回退，断流会自动重连，5 次失败自动换台）。'
          + '也可以在对话里直接说「放个爵士电台」，agent 会用 music_play_plus 的 radio / radioCategory 参数点播。'),
      );
    }
    ...
    function apply(ctx) {
      ...
      const intentTimer = setInterval(() => {
        jsonGet('/dsh-music-plus/intent').then((intent) => {
          ...
          // play a radio station (agent 点名电台)：按需拉一次电台库，然后在该台所属
          // 风格里播放，使 next/prev 在这个风格内走。
          if (intent.radioId) {
            void (async () => {
              if (radioSnap === null) await loadRadio();
              const st = radioStationById(intent.radioId);
              if (st !== null) playStation(st.id, st.categoryId);
            })();
            return;
          }
      ...
      // 网络电台：界面放在播放面板的「网络电台」页签里（本地音乐 / 播客 / 网络电台 /
      // 系统配置），与插件其它设置在同一处，操作统一。原先注册到 DSH 设置页
      // （settings.section）的那一份已按用户要求移除，避免同一功能两处入口。
    ...
    const PLAYER_CSS = '\n' +
      ...
      // ---- 网络电台（设置 → 网络电台）----
      // 全部样式沿用插件既有约定：8px 圆角、layer-1/2 底色、border-l1 描边、
      // 强调色用 --dsh-music-accent（跟随 DSH 主题品牌色）。
      '.dsh-music-radio { gap: 12px; }\n' +
      '.dsh-music-radio-stats { display: flex; flex-wrap: wrap; align-items: center; gap: 4px 12px; font-size: 12px; color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
      '.dsh-music-radio-dim { color: var(--dsw-alias-label-tertiary, rgba(255,255,255,0.45)); font-style: normal; }\n' +
      '.dsh-music-radio-dim.small { font-size: 11px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\n' +
      '.dsh-music-radio-spacer { flex: 1; min-width: 0; }\n' +
      '.dsh-music-settings-btn.small { padding: 3px 9px; font-size: 12px; }\n' +
      '.dsh-music-settings-btn[disabled] { opacity: 0.5; cursor: default; }\n' +
      '.dsh-music-radio-tools { display: flex; flex-wrap: wrap; gap: 8px; align-items: center; }\n' +
      '.dsh-music-radio-search { box-sizing: border-box; flex: 1; min-width: 150px; padding: 6px 10px; border-radius: 8px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.3)); background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); color: var(--dsw-alias-label-primary, #e6e6e6); font-size: 13px; outline: none; }\n' +
      '.dsh-music-radio-search:focus { border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      // 风格选择条：横向可换行的胶囊（与设置页其它 chip 风格一致）。
      '.dsh-music-radio-chips { display: flex; flex-wrap: wrap; gap: 6px; }\n' +
      '.dsh-music-radio-chip { flex: none; padding: 4px 10px; border-radius: 12px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.35)); background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; font-size: 12px; white-space: nowrap; transition: background 0.15s, color 0.15s; }\n' +
      '.dsh-music-radio-chip:hover { color: var(--dsw-alias-label-primary, #e6e6e6); }\n' +
      '.dsh-music-radio-chip.active { background: var(--dsh-music-accent, #2f9e6e); border-color: var(--dsh-music-accent, #2f9e6e); color: var(--dsh-music-accent-fg, #fff); font-weight: 600; }\n' +
      '.dsh-music-radio-msg { padding: 6px 10px; border-radius: 8px; font-size: 12px; }\n' +
      '.dsh-music-radio-msg.ok { background: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 16%, transparent); color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-msg.err { background: color-mix(in srgb, #e5534b 16%, transparent); color: var(--dsw-alias-state-error-primary, #e5534b); }\n' +
      // 电台列表：独立滚动区（设置页里最长的一块），分组标题吸顶。
      '.dsh-music-radio-list { display: flex; flex-direction: column; gap: 1px; max-height: 46vh; overflow-y: auto; padding: 4px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 8px; background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); }\n' +
      '.dsh-music-radio-group { position: sticky; top: 0; z-index: 1; padding: 6px 6px 3px; font-size: 11px; font-weight: 600; color: var(--dsw-alias-label-secondary, #8a8f98); background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); }\n' +
      '.dsh-music-radio-row { display: flex; align-items: center; gap: 6px; padding: 4px 6px; border-radius: 6px; font-size: 12px; color: var(--dsw-alias-label-primary, #e6e6e6); }\n' +
      '.dsh-music-radio-row:hover { background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.06)); }\n' +
      '.dsh-music-radio-row.current { background: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 14%, transparent); }\n' +
      '.dsh-music-radio-row.current .dsh-music-radio-name { color: var(--dsh-music-accent, #2f9e6e); font-weight: 600; }\n' +
      '.dsh-music-radio-row.hls { opacity: 0.55; }\n' +
      '.dsh-music-radio-dot { flex: none; width: 8px; height: 8px; border-radius: 50%; background: var(--dsw-alias-label-tertiary, rgba(255,255,255,0.3)); }\n' +
      '.dsh-music-radio-dot.ok { background: #2f9e6e; }\n' +
      '.dsh-music-radio-dot.dead { background: var(--dsw-alias-state-error-primary, #e5534b); }\n' +
      '.dsh-music-radio-play { flex: none; width: 22px; height: 22px; padding: 0; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.3)); border-radius: 50%; background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; font-size: 10px; line-height: 1; }\n' +
      '.dsh-music-radio-play:hover { color: var(--dsh-music-accent, #2f9e6e); border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-play[disabled] { opacity: 0.4; cursor: default; }\n' +
      '.dsh-music-radio-name { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\n' +
      '.dsh-music-radio-badge { flex: none; padding: 0 5px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 6px; font-size: 11px; line-height: 16px; color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
      '.dsh-music-radio-badge.warn { color: #d9a441; border-color: #d9a441; }\n' +
      '.dsh-music-radio-badge.danger { color: var(--dsw-alias-state-error-primary, #e5534b); border-color: var(--dsw-alias-state-error-primary, #e5534b); }\n' +
      '.dsh-music-radio-badge.accent { color: var(--dsh-music-accent, #2f9e6e); border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-link { flex: none; font-size: 11px; color: var(--dsw-alias-label-secondary, #8a8f98); text-decoration: none; }\n' +
      '.dsh-music-radio-link:hover { color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-star { flex: none; width: 20px; height: 20px; padding: 0; border: none; background: transparent; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; font-size: 13px; line-height: 1; }\n' +
      '.dsh-music-radio-star.on { color: #d9a441; }\n' +
      '.dsh-music-radio-mini { flex: none; width: 20px; height: 20px; padding: 0; border: none; background: transparent; border-radius: 4px; color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; font-size: 12px; line-height: 1; }\n' +
      '.dsh-music-radio-mini:hover { background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.08)); color: var(--dsh-music-accent, #2f9e6e); }\n' +
      // 折叠区（添加电台 / 电台源 / 风格管理）
      '.dsh-music-radio-sections { display: flex; flex-direction: column; gap: 6px; }\n' +
      '.dsh-music-radio-sechead { text-align: left; padding: 8px 10px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 8px; background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); color: var(--dsw-alias-label-primary, #e6e6e6); cursor: pointer; font-size: 13px; font-weight: 600; }\n' +
      '.dsh-music-radio-sechead.open { border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-secbody { display: flex; flex-direction: column; gap: 8px; padding: 10px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 8px; background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); }\n' +
      '.dsh-music-radio-field { display: flex; flex-direction: column; gap: 4px; }\n' +
      '.dsh-music-radio-field label { font-size: 11px; color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
      '.dsh-music-radio-field input, .dsh-music-radio-field select { box-sizing: border-box; width: 100%; padding: 6px 10px; border-radius: 8px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.3)); background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.06)); color: var(--dsw-alias-label-primary, #e6e6e6); font-size: 13px; outline: none; }\n' +
      '.dsh-music-radio-field input:focus, .dsh-music-radio-field select:focus { border-color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-radio-srccard { display: flex; flex-direction: column; gap: 6px; padding: 8px 10px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.2)); border-radius: 8px; background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.04)); font-size: 12px; }\n' +
      '.dsh-music-radio-stylerow { display: flex; align-items: center; gap: 6px; padding: 4px 6px; border-radius: 6px; font-size: 12px; }\n' +
      '.dsh-music-radio-stylerow:hover { background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.06)); }\n' +
      '.dsh-music-radio-stylename { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\n' +
      // 播放面板里的「网络电台」页签：整页一列铺满，台列表自己滚动（这样下面的
      // 添加电台 / 电台源 / 风格管理即使台列表很长也始终够得着）。
      '.dsh-music-radio-pane { flex: 1; min-height: 0; display: flex; flex-direction: column; overflow-y: auto; }\n' +
      '.dsh-music-radio-pane .dsh-music-radio { flex: 1; min-height: 0; }\n' +
      '.dsh-music-radio-pane .dsh-music-radio-list { flex: 1; min-height: 150px; max-height: none; }\n' +
      // 窄屏（手机）：行内允许换行、隐藏主页链接与次要徽标，保证台名与按钮不挤压。
      '@media (max-width: 560px) { .dsh-music-radio-row { flex-wrap: wrap; } .dsh-music-radio-link { display: none; } .dsh-music-radio-list { max-height: 40vh; } .dsh-music-radio-badge { font-size: 10px; line-height: 15px; } }\n';
