...
- **网络电台**：点播放条上的「列表」打开播放面板，切到最上面的「网络电台」页签。内置 16 个风格共 432 台，按风格或搜索挑一台，点行首 ▶ 即用播放条播放；也可以在「＋ 添加 / 编辑电台」「电台源」里自己加台或整批导入外部电台源。
...
入口：播放条上的「列表」→ 打开播放面板 → 顶部「网络电台」页签（与 本地音乐 / 播客 / 系统配置 并列）。电台的全部界面都在这一页，与插件其它设置在同一处。
